...
def constructBayesNet(gameState: hunters.GameState):
    ...
    xCoordinates = list(range(X_RANGE))
    yCoordinates = list(range(Y_RANGE))
    possibleCoordinates = []
    for i in range(len(xCoordinates)):
        for j in range(len(yCoordinates)):
            possibleCoordinates.append((xCoordinates[i], yCoordinates[j]))
    variableDomainsDict[PAC] = possibleCoordinates
    variableDomainsDict[GHOST0] = possibleCoordinates
    variableDomainsDict[GHOST1] = possibleCoordinates
    variableDomainsDict[OBS0] = list(range(X_RANGE - 1 + Y_RANGE - 1 + MAX_NOISE + 1))
    variableDomainsDict[OBS1] = list(range(X_RANGE - 1 + Y_RANGE - 1 + MAX_NOISE + 1))
